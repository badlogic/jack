...
public final class String implements Comparable<String>, CharSequence,
	...
	public String(byte bytes[], int offset, int length, String charsetName)
		...
		if (!(charsetName.equalsIgnoreCase(UTF_8_ENCODING) || charsetName
				.equalsIgnoreCase(ISO_8859_1_ENCODING))) {
	...
	public String(byte bytes[], int highByte, int offset, int length) {
		if (offset < 0 || offset + length > bytes.length) {
			throw new IndexOutOfBoundsException(offset + " < 0 or " + offset
					+ " + " + length + " > " + bytes.length);
		}
	...
	public int compareToIgnoreCase(String s) {
		...
		while (idx < end) {
			if ((result = Character.toLowerCase(charAt(idx))
	...
	public String substring(int start, int end) {
		if (start >= 0 && end >= start && end <= length) {
			...
			throw new IndexOutOfBoundsException(start + " not in [0, " + end
					+ ") or " + end + " > " + length);
		}
	}
	...
	public byte[] getBytes() {
		try {
			...
			throw new RuntimeException("Default '" + DEFAULT_ENCODING
					+ "' encoding not handled", ex);
		}
	}
	...
	public byte[] getBytes(String format)
		...
		} else if (ISO_8859_1_ENCODING.equals(fmt)
				|| LATIN_1_ENCODING.equals(fmt)) {
			return Iso88591.encode((char[]) data, offset, length);
		} else {
			throw new java.io.UnsupportedEncodingException("Encoding " + format
					+ " not supported");
		}
	}
	...
	public String replace(CharSequence target, CharSequence replace) {
		...
		while ((index = unhandled.indexOf(targetString)) != -1) {
			returnValue.append(unhandled.substring(0, index)).append(
					replaceString);
