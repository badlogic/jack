...
public class PersistentSet<T> implements Iterable<T> {
		...
		while (ancestors != null && ancestors.value.red) {
			if (ancestors.value == ancestors.next.value.left) {
				if (ancestors.next.value.right.red) {
					...
					ancestors.next.value.right = new Node(
							ancestors.next.value.right);
				...
				if (ancestors.next.value.left.red) {
					ancestors.value.red = false;
					ancestors.next.value.left = new Node(
							ancestors.next.value.left);
		...
		if (!dead.red) {
			...
			while (ancestors != null && !child.red) {
				if (child == ancestors.value.left) {
					Node<T> sibling = ancestors.value.right = new Node(
							ancestors.value.right);
					if (sibling.red) {
						sibling.red = false;
					...
					Node<T> sibling = ancestors.value.left = new Node(
							ancestors.value.left);
					if (sibling.red) {
						sibling.red = false;
						ancestors.value.red = true;
	...
	private static <T> Cell<Node<T>> minimum(Node<T> n, Cell<Node<T>> ancestors) {
	...
	public Path<T> find(T value) {
		...
		while (old != NullNode) {
			...
			if (difference < 0) {
				...
				return new Path(false, new_, new PersistentSet(newRoot,
						comparator, size), ancestors.next);
			}
		}

		new_.value = value;
		return new Path(true, new_,
				new PersistentSet(newRoot, comparator, size), ancestors);
	}
