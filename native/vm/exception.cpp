...
void exception_retry(void) {
  ...
  exception_handler* handler = exception_get_handler((exception_state)~EXCEPTION_THROWN);
...
exception_handler* exception_add_handler(register exception_state status, register bool *flag, register const char *file, register int line, register const char *function) {
  ...
  exception_handler* handler = (exception_handler*)malloc(sizeof(exception_handler) + function_size + 1);
  if(handler) {
    handler->flag = flag;
    ...
    handler->stack = (unsigned char*)malloc(handler->stack_size);
    handler->file = file;
    handler->line = line;
    if(function) {
    ...
    if(handler->stack) {
      ...
      if(young) {
        ...
        memcpy((char*)young->start - young->stack_size, young->stack, young->stack_size);
...
_Thread_local exception_handler* exception_stack = 0;
