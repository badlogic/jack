...
package com.badlogic.jack.build;

import java.util.ArrayList;

import com.badlogic.jack.build.BuildTarget.TargetOs;
import com.badlogic.jack.build.FileDescriptor.FileType;
...
public class AntScriptGenerator {
	...
	public void generate (BuildConfig config, BuildTarget... targets) {
		...
		String template = new FileDescriptor("com/badlogic/jack/build/resources/scripts/build.xml.template", FileType.Classpath)
			.readString();
	...
	private void copyJniHeaders (String jniDir) {
		final String pack = "com/badlogic/jack/build/resources/headers";
	...
	private String getSharedLibFilename (TargetOs os, boolean is64Bit, String sharedLibName) {
		...
		if (os == TargetOs.Windows) {
			libSuffix = (is64Bit ? "64" : "") + ".exe";
		}
		if (os == TargetOs.Linux || os == TargetOs.Android) {
			libPrefix = "";
			libSuffix = (is64Bit ? "64" : "") + "";
		}
	...
	private String generateBuildTargetTemplate (BuildConfig config, BuildTarget target) {
		...
		if (target.os == TargetOs.Android) {
			...
			String template = new FileDescriptor("com/badlogic/jack/build/resources/scripts/build-android.xml.template",
		...
		String template = new FileDescriptor("com/badlogic/jack/build/resources/scripts/build-target.xml.template",
			FileType.Classpath).readString();
